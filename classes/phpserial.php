...
class phpSerial
{
  ...
  var $_buffer = "";
  var $_os = "";
  var $_winModes = array("parity" => "none",
			 "baud" => "57600",
			 "data" => "8",
			 "stop" => "1",
			 "octs" => "off",
...
//=================================== Lock Port  =========================================//
  ...
  function lockPort ()
  {
    ...
    if ($this->_os === "linux")
      {
	$ret = $this->_exec("lockdev -l " . $this->_device, $out);
	if ($ret)
	  {
	    echo "lockdev lock failed" . PHP_EOL;
	    return false;
	  }
      }
...
//================================= Unlock Port ===========================================//
  ...
  function unlockPort()
  {
    ...
    if ($this->_os === "linux")
      {
	$ret = $this->_exec("lockdev -u " . $this->_device, $out);
	if ($ret)
	  {
	    echo "lockdev unlock failed" . PHP_EOL;
	    return false;
	  }
      }
    return true;
...
//=============================================================================
  ...
  function get_win_mode_string()
  {
    //piece together assoc array keys and values with '='
    $mode_array = array_map(function($key, $value)
			    { return $key . "=" . $value . " "; },
			    array_keys($this->_winModes),
			    array_values($this->_winModes));
    return implode($mode_array);//piece together string of all values
  }
  ...
  function confBaudRate ($rate)
  {
	...
	elseif ($this->_os === "windows")
	  {
	    $this->_winModes["baud="] = $validBauds[$rate];//change baud

	    $mode_string = $this->get_win_mode_string();

	    $ret = $this->_exec("mode " . $this->_windevice . $mode_string,
				$out);
	  }
	else return false;
  ...
  function confParity ($parity)
  {
	...
	$ret = $this->_exec("stty -f " . $this->_device . " " . $args[$parity], $out);
      ...
      {
	$this->_winModes["parity"] = $parity{0};//change baud
  ...
  function confCharacterLength ($int)
  {
	...
	$ret = $this->_exec("stty -f " . $this->_device . " cs" . $int, $out);
      ...
      {
	$this->_winModes["data"] = $int;//change baud
  ...
  function confStopBits ($length)
  {
	...
	$ret = $this->_exec("stty -f " . $this->_device . " " . (($length == 1) ? "-" : "") . "cstopb", $out);
      ...
      {
	$this->_winModes["stop"] = $length;//change baud
  ...
  function confFlowControl ($mode)
  {
	...
	switch($mode)
	  {
	  case "none":
	    $this->_winModes["xon"] = "off";//change baud
	    $this->_winModes["octs"] = "off";//change baud
	    $this->_winModes["rts"] = "on";//change baud
	    break;
	  case "rts/cts":
	    $this->_winModes["xon"] = "off";//change baud
	    $this->_winModes["octs"] = "on";//change baud
	    $this->_winModes["rts"] = "hs";//change baud
	    break;
	  case "xon/xoff":
	    $this->_winModes["xon"] = "on";//change baud
	    $this->_winModes["octs"] = "off";//change baud
	    $this->_winModes["rts"] = "on";//change baud
	    break;
	  }
   ...
   *
   * @param string $mode Set the transfer mode. Availible modes :
   * 	-> "ASCII" : special characters allowed
   * 	-> "binary" : no special characters
   * @return bool
   */
  function confTransferMode ($mode)
  {
    if ($this->_dState !== SERIAL_DEVICE_SET)
      {
	trigger_error("Unable to set transfer mode : the device is either not set or opened", E_USER_WARNING);
	return false;
      }


    if ($mode !== "ASCII" and $mode !== "binary") {
      trigger_error("Invalid transfer mode specified", E_USER_ERROR);
      return false;
    }

    $linuxModes = 
      array("ASCII"  =>
	    "-ignbrk icrnl opost isig icanon iexten echo",
	    "binary" =>
	    "ignbrk -icrnl -opost -isig -icanon -iexten -echo"
	    );
    ...
    if ($ret === 0) return true;
      ...
      {
	trigger_error("Unable to set transfer mode : " . $out[1], E_USER_ERROR);
	return false;
      }
  }
